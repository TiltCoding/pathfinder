...
- **One card + one choice per feature, sharing `id = feat-K`.** For each top-K candidate `K` write:
  - a `planBlocks[]` card `{ "id": "feat-K", "title": "<feature name>", "body": "<markdown>" }` whose
    ...
    the existing `body` field, per ADR-0013). **Preferred (feat-18):** also/instead set the structured
    `planBlocks[].rank = { "score", "agreement", "impact", "effort", "risk" }` — the dashboard renders it
    as a compact chip in the card header (`agreement` is a 0–1 fraction shown as N%). It degrades
    gracefully: absent `rank` (old runs / non-improve gates) renders unchanged, so the markdown line stays
    a valid fallback. This is the rich context the human reads, and
