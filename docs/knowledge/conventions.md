...
- **Язык вывода = глобальная настройка плагина** (`~/.claude/ai-pathfinder/settings.json`, `en` при
  отсутствии — **eng-first**); **в чате/реплаях язык вопроса перебивает дефолт** (авто-детект). Раньше
  было жёсткое «всё человекочитаемое → русский» — заменено задачей `dashboard-i18n` (ADR-0018). Детали:
  **язык вопроса** для `replies.json`/`chat.jsonl`/`/ask` summary; **глобальный дефолт** для
  brief/exploration/plan/questions/docs/гейтов/опций/коммитов; **стабильно английские** машинно-парсимые
  заголовки/ключи (секции дайджестов, `cand:`-ключи). UI дашборда/хаба локализован (`en`/`ru`,
  `areas/dashboard-i18n.md`); эта база знаний — на русском **по факту настройки**, а не по правилу.
