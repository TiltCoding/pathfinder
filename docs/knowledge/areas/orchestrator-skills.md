...
> `/design`, `/test`, `/docs`) + роутер `/start`, и их под-агенты: регистрация конвенцией каталогов, паттерн «SKILL.md + reference-файлы»,
...
- **`/docs`** — **только-документация для области** (не вопрос-ответ, не фича): человек называет область →
  рой read-only `ask-researcher` сверяет area-доки/README/ADR/`INDEX.md` с **кодом** и даёт drift-список
  (`{doc path:line, claim, code path:line, reality, fix}`) → COMPOSE правок → **гейт плана** (дифф доков,
  «Изменения») → `wf-documenter` пишет и рефрешит `INDEX.md` (`skills/docs/SKILL.md:1`). Машина
  `INTAKE→AUDIT→COMPOSE→PLAN GATE→WRITE→DONE`. **Реюзит ростер** (ask-researcher + wf-documenter) — нового
  агента нет; **не меняет поведение** кода (нашёл код-баг → не чинит); инвариант **eng-first** для
  docs/README. `description` разведён от ask/feature/improve. Седьмая полноценная команда.
...
- **«Утвердить» вбирает ответы (ADR-0026).** `draft.json` **не** в `READABLE_FILES` сервера, поэтому выбор
  по-прежнему читается только из `submissions/<n>.json` — **но** кнопка «Утвердить план» теперь сама
  авто-`/submit`'ит неотправленный черновик перед `approve-plan` (общий `flushDraft()`), так что approve
  почти всегда несёт свежий сабмишн. На гейте `/feature` это **один клик** при чистых выборах (инлайн ask-row
  при свободной правке/открытом треде); на гейте `/improve` — первый из двух армированных кликов делает
  `flushDraft()`, второй диспатчит. **Ключевое для оркестратора:** `approve-plan` и новый сабмишн часто
  приходят в **одном** `/wait` — применить ответы, затем идти **прямо в IMPLEMENT/DISPATCH без ре-парка**.
  Fallback: если `approve-plan` пришёл вовсе без свежего сабмишна — читать нечего, переспросить человека.
  Дефолт «**нет ответа = Пропускаем**»: фича без `answer` не диспетчится (`saveAnswer` игнорит пустой ввод).
  Убраны `submittedOnce`/степпер `①②`/нудж `submitFirst` (`skills/improve/dashboard-guide.md` §SELECT GATE).
